...
var SFAddressHandler=function(country_id,state_id){var self=this;var x,i=0;var country;this.country_sel=document.getElementById(country_id);this.state_sel=document.getElementById(state_id);var initial_country=this.country_sel.getAttribute('data-selected');for(country in countries){if(countries.hasOwnProperty(country)){if(iso||initial_country.length==2){this.country_sel.options[i]=new Option(country,countries[country].value);}
...
i++;}}
this.country_sel.addEventListener('change',function(e){self.updateStates(this.options[this.selectedIndex].value);});};SFAddressHandler.prototype.updateStates=function(country_name){var country;var region_names;var regions;var i=0;var initial_state=this.state_sel.getAttribute('data-selected');while(this.state_sel.options.length>0){this.state_sel.remove(0);}
...
country_select_name=country_select_name||'00Nj0000005FAeV';state_select_name=state_select_name||'00Nj0000005FAiE';$("select[name='"+ country_select_name+"']").each(function(index){var addressForm=new SFAddressHandler(this.id,$(this).closest('form').find("select[name='"+ state_select_name+"']").attr('id'));var $countrySel=$(this);if($countrySel.data('selected')){$countrySel.val($countrySel.data('selected'));}else{$countrySel.val("United States");}
